            ...
            protected void onBindViewHolder(@NonNull OrderViewHolder viewHolder, int position, @NonNull Request model) {
                ...
                viewHolder.btndeleteOrder.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        // Change the order status to 3 (cancelled) in the Firebase Realtime Database
                        DatabaseReference orderRef = adapter.getRef(position);
                        orderRef.child("status").setValue("3");

                        // Disable and hide the button after clicking
                        viewHolder.btndeleteOrder.setEnabled(false);
